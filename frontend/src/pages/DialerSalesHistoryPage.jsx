...
import {
  ...
  CalendarDays, Search, Users, TrendingUp, Phone, Filter, X, Ban, CalendarRange, UserCheck
} from 'lucide-react';
...
function DateRangeDropdown({ startDate, endDate, onChange }) {
  ...
  return (
    <div className="relative animate-in fade-in duration-300" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-64 bg-slate-900 border border-slate-750 rounded-lg shadow-xl z-50 overflow-hidden">
          <div className="p-1">
            {presets.map((preset) => (
              <button
                key={preset.label}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-800 border border-slate-700 text-slate-200 text-[10px] rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-violet-500"
                  />
                </div>
                <div>
