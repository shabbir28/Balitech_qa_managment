...
import {
  ...
  Search, Users, TrendingUp, Phone, Filter, X, Ban, UserCheck
} from 'lucide-react';
...
export default function AgentSalesPage() {
  ...
  }, [dialerType, startDate, endDate, agentQuery]);
  ...
  return (
    <div className="space-y-4 max-w-[1400px] mx-auto pb-6">
      ...
      {/* ─── HEADER ─────────────────────────────────────────── */}
      <div className="bg-gradient-to-br from-slate-900 via-[#101423] to-indigo-950/20 rounded-2xl border border-slate-700/50 p-5 lg:p-6 shadow-xl relative flex flex-col gap-5">
        {/* Decorative elements (must be overflow-hidden but separate from content) */}
        <div className="absolute inset-0 overflow-hidden rounded-2xl pointer-events-none">
          <div className="absolute -right-20 -top-20 w-64 h-64 bg-indigo-500/10 rounded-full blur-[80px]" />
          <div className="absolute -left-10 -bottom-10 w-40 h-40 bg-emerald-500/10 rounded-full blur-[60px]" />
        </div>
        
        {/* Top Row: Title & Action */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-gradient-to-br from-indigo-500/20 to-purple-500/10 rounded-xl border border-indigo-500/30 shadow-inner">
              <Users className="w-6 h-6 text-indigo-400 drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-300 tracking-tight leading-tight mb-1">
                Agent Wise Sales
              </h1>
              <p className="text-slate-400 text-xs flex items-center gap-1.5 font-medium">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span>
                {fetched ? dateRangeLabel : 'Filter & track performance by specific agents'}
              </p>
            </div>
          </div>
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center gap-3 bg-slate-950/40 p-2.5 rounded-xl border border-slate-700/50 shadow-inner">
          {/* Dialer Select */}
          <div className="relative w-full lg:w-48 shrink-0">
            <select
              ...
              className="w-full appearance-none bg-slate-900 border border-slate-700 text-slate-200 text-sm rounded-lg pl-3 pr-8 py-2 h-[42px] focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed shadow-sm transition-all"
            >
          ...
          {/* Agent Input */}
          <div className="relative flex items-center w-full lg:flex-1 shrink-0">
            <Users className="w-4 h-4 text-slate-400 absolute left-3.5" />
            <input
              type="text"
              placeholder="Enter Agent Name or ID (e.g. Mahaz)"
              value={agentQuery}
              onChange={e => setAgentQuery(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 text-slate-100 text-sm rounded-lg pl-10 pr-4 py-2 h-[42px] focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 placeholder:text-slate-500 shadow-sm transition-all"
              onKeyDown={e => { if (e.key === 'Enter') fetchHistory(); }}
            />
          </div>

          {/* Date Range Dropdown */}
          <div className="w-full lg:w-64 shrink-0 h-[42px] relative z-20">
            <DateRangeDropdown
              startDate={startDate}
              endDate={endDate}
          ...
          <button
            ...
            disabled={loading}
            className="flex w-full lg:w-auto items-center justify-center gap-2 px-6 h-[42px] bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white rounded-lg text-sm font-bold transition-all shadow-lg hover:shadow-indigo-500/25 active:scale-95 disabled:opacity-50 disabled:pointer-events-none shrink-0"
          >
      ...
      {/* ─── EMPTY STATE ─────────────────────────────────────── */}
      {!fetched && !loading && (
        <div className="flex flex-col items-center justify-center py-28 text-slate-400 gap-4 max-w-md mx-auto text-center">
          <div className="w-20 h-20 bg-slate-900/80 rounded-3xl flex items-center justify-center shadow-xl border border-slate-800/60 relative">
             <div className="absolute inset-0 bg-indigo-500/20 blur-xl rounded-full" />
             <Users className="w-10 h-10 text-indigo-400 relative z-10 drop-shadow-md" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-slate-200 mb-1">Find Agent Sales</h3>
            <p className="text-sm font-medium text-slate-500 leading-relaxed">
              Enter an agent's name or ID and select a date range above to view all their associated sales instantly.
            </p>
          </div>
        </div>
      )}
