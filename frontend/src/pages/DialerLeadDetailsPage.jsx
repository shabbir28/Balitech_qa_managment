...
import { ArrowLeft, User, Phone, Play, Calendar, AlertCircle, Download } from 'lucide-react';
...
export default function DialerLeadDetailsPage() {
    ...
    try {
      if (assignmentId) {
        ...
        if (response.data.success) {
          ...
          navigate(`/evaluations/new?call_id=${assignmentId}&lead_id=${leadId}&dialer=${encodeURIComponent(dialerParam)}&assignment_id=${assignmentId}`, {
            state: { recordings, initialRecordingUrl: recording.location, assignmentId }
          });
        }
      } else {
        ...
        if (response.data.success) {
          toast.success('Ready to evaluate!');
          navigate(`/evaluations/new?call_id=${response.data.call_id}&lead_id=${leadId}&dialer=${encodeURIComponent(dialerParam)}${teamParam ? `&team=${encodeURIComponent(teamParam)}` : ''}${assignmentId ? `&assignment_id=${assignmentId}` : ''}`, {
            state: { recordings, initialRecordingUrl: recording.location, assignmentId }
          });
        }
      }
  ...
  const handleBack = () => {
    // If user came from assignments, has an assignment_id, or is a QA Agent, return directly to My Assignments
    if (assignmentId || user?.role === 'QA Agent') {
      navigate('/my-assignments');
      return;
    }
    // If opened from dialer search, go back to dialer search
    if (searchParams.get('from') === 'search') {
      navigate('/dialer');
      return;
    }
    // Otherwise fallback to dialer sales
    navigate('/dialer-sales');
  };
                ...
                {recordings.map((rec, idx) => (
                  <div key={idx} className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-4 hover:border-emerald-500/30 transition-colors group">
                    ...
                    <div className="shrink-0 flex sm:flex-col justify-end gap-2 mt-4 sm:mt-0">
                      <a
                        href={rec.location}
                        download={rec.filename || `recording_${idx + 1}.mp3`}
                        target="_blank"
                        rel="noreferrer"
                        className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-semibold rounded-lg border border-slate-700 transition-all flex items-center justify-center gap-1.5"
