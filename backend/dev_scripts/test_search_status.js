...
async function run() {
  try {
    const body = new URLSearchParams({ status: 'SALE', SUBMIT: 'SUBMIT' }).toString();
    const html = await fetchAdminPage('admin_search_lead.php', 'medicare', 'POST', body);
    require('fs').writeFileSync('search_status_sale.html', html);
    ...
    if (rows) {
      for (const row of rows) {
        if (row.includes('admin_modify_lead.php?lead_id=')) {
          count++;
        }
      }
    }
    console.log(`Found ${count} leads with status SALE in medicare dialer`);
