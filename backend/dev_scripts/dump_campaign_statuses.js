...
async function run() {
  try {
    const html = await fetchAdminPage('admin.php?ADD=34&campaign_id=001', 'medicare');
    require('fs').writeFileSync('campaign_statuses.html', html);
    console.log('Saved campaign_statuses.html');
  } catch (err) {
    console.error(err);
  }
}
