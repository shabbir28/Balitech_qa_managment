...
async function fixAgentNames() {
  try {
    ...
    // Fetch data from August 1st to today
    const params = new URLSearchParams();
    params.append('run_export', '1');
    params.append('query_date', '2026-08-01');
    params.append('end_date', '2026-08-15');
    ...
    console.log('Fetching data from Vicidial...');
    const tsvData = await fetchAdminPage('call_report_export.php', 'medicare', 'POST', params.toString());
    
    const lines = tsvData.split('\n');
    let updatedCount = 0;

    console.log(`Received ${lines.length} lines from Vicidial. Processing updates...`);

    // Create a map of phone -> agent string
    // We'll update dialer_sales_history where phone matches and agent is just the name
    for (const line of lines) {
      ...
      if (cols.length >= 10 && cols[0] && cols[0].trim().startsWith('20')) {
        ...
        if (phone && agentName && agentId) {
          ...
          // Only update if current agent != combinedAgent
    ...
    console.log(`Successfully updated ${updatedCount} records in the database with Agent IDs!`);
